...
class MoonshineStats extends StatelessWidget {
  const MoonshineStats({
    super.key,
  });

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<MoonshineStatsBloc, MoonshineStatsState>(
      builder: (context, state) {
        if (state.status == MoonshineStatsStatus.loading) {
          return const Center(
            child: CircularProgressIndicator(),
          );
        } else if (state.status == MoonshineStatsStatus.error) {
          return const Center(
            child: Text('Error'),
          );
        }
        ...
        // for (final s in state.runnersList) {
        //   data.add(
        //     ScatterSpot(
        //       s['bags'],
        //       s['money'],

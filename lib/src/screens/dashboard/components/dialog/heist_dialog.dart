...
heistDialog({
  ...
  return showDialog<String>(
    context: context,
    builder: (BuildContext context) => HeistAlertDialog(
      context: context,
...
class HeistAlertDialog extends StatefulWidget {
  const HeistAlertDialog({
    super.key,
  ...
  @override
  State<HeistAlertDialog> createState() => _HeistAlertDialogState();
}

class _HeistAlertDialogState extends State<HeistAlertDialog> {
  List<Widget> list = [];
  ...
  Widget getActionWidget(DialogState state) {
    return SizedBox(
      width: double.infinity,
      child: DropdownButton(
        value: state.heistAction.isEmpty
            ? Data.getActions(widget.organization, 'Heist').first
            : state.heistAction,
        items: Data.getActions(widget.organization, 'Heist')
            .map<DropdownMenuItem<String>>((String value) {
          return DropdownMenuItem<String>(
            value: value,
            child: Text(value),
        ...
        onChanged: (String? value) {
          print(value);
          context.read<DialogCubit>().heistActionChanged(value!);
        },
      ),
    );
  }
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                ...
                context
                    .read<DashboardBloc>()
                    .add(DashboardCreateHeistActivity(heistActivity));
